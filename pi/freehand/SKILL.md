...
where an independent reviewer attacks the work and you incorporate its feedback
until it is satisfied or the round cap is hit. The reviewer tries to prove the
implementation does not do what was asked; your job is to leave it nothing to
find.

The reviewer is not a subagent - pi has none, and this skill needs no plugin.
You **spawn a fresh pi process headlessly** (step 6): it reads its instructions
and the review contract from a brief you pipe in on stdin, replies with its
verdict on stdout, and exits. Spawned with `--no-session` and no write tools,
it starts with no memory of your session and cannot edit the project - the
independence the loop depends on.
...
handed to the reviewer as the contract. This skill is self-contained: it owns
its reviewer prompt (`freehand-reviewer.md`) and does not depend on any other
skill.
...
6. **Spawn the reviewer.** Assemble the review brief and run a fresh pi process
   to review the work, via the Bash tool:

   1. Read `freehand-reviewer.md` from this skill's directory - it is the
      reviewer's instructions. Append this round's inputs:
      - The **transcript** from step 3, in full and verbatim, every round. It
        has no memory between rounds and nothing on disk to read it from, so
        re-sending it is what makes the review possible at all.
      - Every assumption you decided rather than asked, and every test you
        skipped under step 4, each stated plainly so it can be attacked.
   ...
   2. Write the brief to a temporary file, then spawn pi headlessly with the
      brief piped in on stdin, capturing the response:

   ```bash
   brief=$(mktemp /tmp/freehand-review.XXXXXX.md)
   out=$(mktemp /tmp/freehand-review.XXXXXX.out)
   cat <this-skill-dir>/freehand-reviewer.md > "$brief"
   cat <<'EOF' >> "$brief"
   <this round's inputs: transcript, declared assumptions, declared test skips, work summary, prior feedback>
   EOF
   ...
   - `--tools read,bash,grep,find,ls` keeps it read-only - it can inspect and
     run the project but not edit it.
   - `--no-session` makes the spawn ephemeral.

   The spawn inherits your working directory, context files and skills, so the
   reviewer sees the same project, conventions and tooling you do - including
   the `agent-browser` skill when it must drive a web app.

   Spawn it fresh each round; reviewing your own work inline defeats the
   mechanism. Present the work plainly - do not pre-defend it or steer it
   toward a pass. If the spawn fails to produce a response (e.g. an unknown
   model), retry once with `--model` dropped so it falls back to your
   configured default, then report the failure to the user if it still fails.

7. **Read the verdict line.** The review's response leads with its verdict.
   Read the first line of the captured output:

   - `VERDICT: SATISFIED` ends the loop (go to step 9).
   - `VERDICT: NEEDS_WORK` continues to step 8.

   If no verdict line appears, treat the review as failed and retry the spawn
