...
run an adversarial review loop: you build, an independent reviewer attacks the
work against the spec, you incorporate its feedback - round after round - until
it is satisfied or the round cap is hit. The reviewer tries to prove the
...
You **spawn a fresh pi process headlessly** (step 4): it reads its instructions
and this round's inputs from a brief you pipe in on stdin, replies with its
...
This skill is self-contained: it owns its reviewer prompt (`build-reviewer.md`)
and does not depend on any other skill.
...
4. **Spawn the reviewer.** Assemble the review brief and run a fresh pi process
   to review the work, via the Bash tool:

   1. Read `build-reviewer.md` from this skill's directory - it is the
      reviewer's instructions. Append this round's inputs: the feature
      directory path, a summary of the work across every phase, the full diff
      for the feature, and - from round two onward - the feedback it gave last
      time so it can confirm each point was addressed.
   2. Write the brief to a temporary file, then spawn pi headlessly with the
      brief piped in on stdin, capturing the response:

   ```bash
   brief=$(mktemp /tmp/build-review.XXXXXX.md)
   out=$(mktemp /tmp/build-review.XXXXXX.out)
   cat <this-skill-dir>/build-reviewer.md > "$brief"
   cat <<'EOF' >> "$brief"
...
5. **Read the verdict line.** The review's response leads with its verdict.
   Read the first line of the captured output:

   - `VERDICT: SATISFIED` ends the loop (go to step 7).
